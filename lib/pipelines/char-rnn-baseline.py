...
parser = argparse.ArgumentParser(description='Baseline Seq2Seq character level model')
...
def seq2seq_char_level(args, input_texts, target_texts):
    # Vectorize the data.
...
def main():
    ...
    df = pd.read_csv(args.file_data_raw).dropna().head(10000)
    print(f'loaded dataset of size {len(df)}')

    # dataset
    input_texts = df['body']

    # method_char_counts = df['body'].progress_apply(len)
    # cutoff_body_length = 2000
    # proportion_of_body_chars_kept = method_char_counts.where(lambda x: x < cutoff_body_length).sum() \
    #     / method_char_counts.sum()
    # print(proportion_of_body_chars_kept)
    # method_char_counts.hist(bins=200)
    # plt.show()

    # the output sequences are marked with special tokens for START and END
    target_texts = df['method_name']

    # print(input_texts.head(5))
    # print('-------')
    # print(target_texts.head(5))
    seq2seq_char_level(args, input_texts, target_texts)
